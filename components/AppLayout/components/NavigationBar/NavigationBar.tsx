...
const NavigationBar = () => {
  ...
  return (
    <section className="sticky top-0 z-50 bg-[#F9FFCD] lg:bg-inherit	 mb-10  lg:backdrop-blur-[5px]">
      <nav className="flex flex-col lg:flex-row justify-start lg:justify-between p-7 lg:px-4 lg:py-0 max-w-7xl mx-auto h-screen lg:h-16 items-start lg:items-center ">
        <div className="mb-5 lg:mb-0 relative w-full lg:w-auto">
          <img src="/codework-logo.png" className="h-4 w-auto" alt="xx" />
           <div className="flex lg:hidden absolute top-0 right-0 text-2xl cursor-pointer" onClick={() => toggleNavbar()}><GiHamburgerMenu/></div> 
        </div>

        <div className={`flex flex-col lg:flex-row items-center justify-end gap-x-10 w-full transition-all  ${!isOpen ? '' : ''}`}>
